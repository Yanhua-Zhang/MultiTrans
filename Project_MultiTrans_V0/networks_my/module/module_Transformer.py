...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dwconv(x)
        x = self.act(x)
        x = self.drop(x)    # 这里这个 dropout 被 droppath 代替 
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class Block_Spatial_Battleneck_Share_spatial_reduction(nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads, Spatial_ratio, key_value_ratio, mlp_ratio=4., attn_ratio=2., drop=0., drop_path=0., one_kv_head = True, share_kv = True, If_efficient_attention = True, act_layer=nn.ReLU, norm_cfg=dict(type='BN2d', requires_grad=True)):
        super().__init__()
        ...
        if self.If_efficient_attention:
            self.attn = efficient_Attention_modified_Param_sharing_Share_spatial_reduction(dim, key_dim=key_dim, num_heads=num_heads, attn_ratio=attn_ratio, one_kv_head = one_kv_head, share_kv = share_kv, activation=act_layer, key_value_ratio=key_value_ratio, norm_cfg=norm_cfg)
        else:
            self.attn = Attention_Param_sharing_Kv_sharing(dim, key_dim=key_dim, num_heads=num_heads, attn_ratio=attn_ratio, one_kv_head = one_kv_head, share_kv = share_kv, activation=act_layer, norm_cfg=norm_cfg)

        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        # 不太明白 nn.Identity() 是干啥的。网上有解释说是占位置用的
